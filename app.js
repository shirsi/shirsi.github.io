...
for (i = 0; i< data.results.length; i++){
  ...
  let gender = $('<li>').text(`Gender:       ${data.results[i].appearance.gender}`)
    ...
    const $work = $('<ul>')
              .append('<h4>Work</h4>')
              .appendTo($infodiv)
    let occupation = $('<li>').append(`<h5>Occupation: </h5>      ${data.results[i].work.occupation}`)
              .appendTo($work)
    let base = $('<li>').append(`<h5>Base: </h5>       ${data.results[i].work.base}`)
              .appendTo($work)
  const $connections= $('<ul>')
                        .append(`<h4>Connections</h4>`)
                        .appendTo($infodiv)
  let group = $('<li>').append(`<h4>Group-affiliation: </h4>      ${data.results[i].connections['group-affiliation']}`)
                        .appendTo($connections)
              let relatives = $('<li>').text(`Relatives:       ${data.results[i].connections.relatives}`)
                        .appendTo($connections)
          let $powerstats = $('<ul>').appendTo($infodiv).addClass('powerstats').append("<h4>Power Stats: </h4>")
          let intelligence = $('<li>')
...
const $h2 = $('<h2>')
  ...
  const $name = $('<h3>')
    ...
    const $ul =$('<ul>').append("<h4>Aliases: </h4>")
    $infodiv.append($ul)
          ...
          const $appearance = $('<ul>')
              ...
              let gender = $('<li>').text(`Gender:       ${data.appearance.gender}`)
                ...
                const $work = $('<ul>')
                          .append(`<h4>Work</h4>`)
                          .appendTo($infodiv)
                let occupation = $('<li>').append(`<h5>Occupation:
</h5>    ${data.work.occupation}`)
                          .appendTo($work)
                let base = $('<li>').append(`<h5> Base:</h5>        ${data.work.base}`)
                          .appendTo($work)
              const $connections= $('<ul>')
                                    .append(`<h4> Connections</h4> `)
                                    .appendTo($infodiv)
              let group = $('<li>').append(`<h5> Group-affiliation: </h5>        ${data.connections['group-affiliation']}`)
                                    .appendTo($connections)
